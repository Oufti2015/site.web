...
var SWFAddress = new function() {
    ...
    var _getHash = function() {
        var index = _l.href.indexOf(asual.swfAddressStart);
        return index != -1 ? _ec(_dc(_l.href.substr(index + asual.swfAddressStart.length))) : '';
    };
    ...
    var _titleCheck = function() {
        if (_d.title != _title && _d.title.indexOf(asual.swfAddressStart) != -1)
            _d.title = _title;
    };
    ...
    if (_supported) {
        ...
        if (_msie && _l.hash != _getHash())
            _l.hash = asual.swfAddressStart + _ieLocal(_getHash(), TRUE);
    ...
    } else if ((!_supported && _l.href.indexOf(asual.swfAddressStart) != -1) || 
        (_safari && _version < 418 && _l.href.indexOf(asual.swfAddressStart) != -1 && _l.search != '')){
        _d.open();
        _d.write('<html><head><meta http-equiv="refresh" content="0;url=' + 
            _l.href.substr(0, _l.href.indexOf(asual.swfAddressStart)) + '" /></head></html>');
        _d.close();
    } else {
        _track();
    }
    ...
    this.getBaseURL = function() {
        var url = _l.href;
        if (url.indexOf(asual.swfAddressStart) != -1)
            url = url.substr(0, url.indexOf(asual.swfAddressStart));
    ...
    this.setTitle = function(title) {
        ...
        _st(function() {
            ...
            if (!_justset && _mozilla)
                _l.replace(_l.href.indexOf(asual.swfAddressStart) != -1 ? _l.href : _l.href + asual.swfAddressStart);
            _justset = FALSE;
        }, 10);
    };
    ...
    this.setStatus = function(status) {
        ...
        if (!_safari) {
            ...
            if (!(/http(s)?:\/\//.test(status))) {
                var index = _l.href.indexOf(asual.swfAddressStart);
                status = (index == -1 ? _l.href : _l.href.substr(0, index)) + asual.swfAddressStart + status;
            }
            _t.status = status;
        }
    };
    ...
    this.setValue = function(value) {
        ...
        if (_safari) {
            if (_opts.history) {
                ...
                if (_version < 418) {
                    if (_l.search == '') {
                        _form.action = asual.swfAddressStart + _value;
                        _form.submit();
                    }
                    ...
                    anchor.href = asual.swfAddressStart + _value;
                    anchor.dispatchEvent(evt);                
                } else {
                    _l.hash = asual.swfAddressStart + _value;
                }
            } else {
                _l.replace(asual.swfAddressStart + _value);
            }
        } else if (_value != _getHash()) {
            if (_opts.history)
                _l.hash = asual.swfAddressStart + _dc(_ieLocal(_value, TRUE));
            else
                _l.replace(asual.swfAddressStart + _dc(_value));
        }
    ...
    this.getPath = function() {
        ...
        } else if (value.indexOf(asual.swfAddressStart) != -1) {
            return value.split(asual.swfAddressStart)[0];
        } else {
            return value;   
        }        
    };
